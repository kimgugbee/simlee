...
export default function TciResult() {
  ...
  const data = tciTypes[decodeURIComponent(type)];

  if (!data) {
    return (
      <div className="result-error">
        <p>알 수 없는 유형입니다.</p>
        <button onClick={() => navigate("/tci")}>돌아가기</button>
      </div>
    );
  }

  function handleShare() {
    const text = `나의 TCI 기질 유형은 ${data.name} ${data.title}!\n${data.description}\n\n나도 검사해보기: ${window.location.origin}/tci`;
    if (navigator.share) {
      navigator.share({ title: "TCI 기질 검사 결과", text });
    } else {
  ...
  return (
    <div className="tst-result">
      <div className="tst-result-header" style={{ background: data.gradient }}>
        ...
        <div className="tst-result-badge">TCI 기질 결과</div>
